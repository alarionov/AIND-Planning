...
class AirCargoProblem(Problem):
  ...
  def actions(self, state: str) -> list:
    ...
    for action in self.actions_list:
      if not all(clause in kb.clauses for clause in action.precond_pos):
        continue
  ...
  def result(self, state: str, action: Action):
    ...
    positives = set(current_state.pos)
    negatives = set(current_state.neg)

    for effect in action.effect_add:
      positives.add(effect)
      negatives.remove(effect)

    for effect in action.effect_rem:
      positives.remove(effect)
      negatives.add(effect)

    new_state = FluentState(positives, negatives)
  ...
  def h_ignore_preconditions(self, node: Node):
    ...
    return len([clause for clause in self.goal if clause not in current_state.pos])
